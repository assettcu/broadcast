...
class Flashes {
        ...
        if(!empty($flashes)) {
            foreach($flashes as $key=>$message) {
                ...
                switch($key) {
                    ...
                    default: echo '<div class="ui-widget-content flash ui-corner-all">'.$icon.$iconclose.$message.'</div>'; break;
                }
    ...
    {
        $icon = "<div class='flash-icon'>";
        switch($type) {
            case "success":     $icon .= StdLib::load_image("check-64","16px");             break;
            case "error":       $icon .= StdLib::load_image("attention","16px");            break;
            case "twitter":     $icon .= StdLib::load_image("twitter_logo","16px");         break;
            case "facebook":    $icon .= StdLib::load_image("facebook_logo","16px");        break;
            case "googleplus":  $icon .= StdLib::load_image("googleplus_logo","16px");      break;

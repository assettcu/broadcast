...
class UserObj extends FactoryObj
{
    ...
    public function permission_atleast($position) 
    {
        if(!$this->loaded) {
            return false;
        }
		if(!is_int($position))
			$position = $this->numericalize_position($position);
        return ($this->permission >= $position);
    }
    
    public function numericalize_position($position) 
    {
        switch(strtolower($position)) {
            case "administrator": case "admin": return 3;
            case "manager": return 2;
            case "user": case "basic_user": return 1;
            default: return 0;
        }
    }
	public function getAllUsers() {
		if(!$this->permission_atleast(10)) return null;
	...
	public function getUserInfo() {
        ...
        ";
        $command = $conn->createCommand($query);
		$command->bindParam(":identikey",$this->username);
		return $command->queryAll();
	}
	public function getDepartmentName() {
		...
		if(!$command->queryAll())
			return "";
		if(!$command->queryAll()[0]["deptid"])
			return "";
		$dept_id = $command->queryAll()[0]["deptid"];
        ...
        ";
        $command2 = $conn2->createCommand($query2);
        $command2->bindParam(":dept_id",$dept_id);
		return $command2->queryAll()[0]["deptname"];
	}
}
